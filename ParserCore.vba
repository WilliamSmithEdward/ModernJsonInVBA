...
'
' Summary
'   Core JSON parsing engine extracted from ModernJsonInVBA.
'
'   This file contains the deterministic JSON parser and in-memory data model
'   used by the full library, but intentionally excludes all Excel-specific
'   functionality (ListObjects, worksheet utilities, table extraction, etc.).
'
'   It is intended for:
'
'       • Standalone JSON parsing in pure VBA
'       • Educational/reference purposes
'       • Environments where Excel dependencies are undesirable
'       • Embedding the parser in other VBA projects
'
'
' Included Capabilities
'
'   • Strict JSON parser (RFC 8259 compliant)
'   • Deterministic object representation
'   • Unicode escape and surrogate pair handling
'   • Numeric parsing with validation
'   • JSON stringify (object/array/primitives)
'   • Path resolution helpers
'   • Safe object/array traversal utilities
'
'
' Data Model
'
'   JSON Object
'       ? VBA Collection tagged with TAG_OBJECT in slot(1)
'         followed by key/value pairs stored as Variant(0 To 1)
'
'   JSON Array
'       ? VBA Collection (untagged)
'
'   JSON Primitive
'       ? Native VBA Variant
'           String
...
'
'   • Object key order preserved exactly as parsed
'   • Stringify output is stable for identical inputs
'   • No reliance on Scripting.Dictionary or external libraries
'   • Compatible across Excel, Access, Word, and VB6 hosts
'
'
' Not Included In This File
'
'   The following higher-level features remain in the full
'   ModernJsonInVBA module:
'
'       • Excel ListObject upsert engine
'       • JSON ? Excel table extraction
'       • Nested array table projection
'       • Excel ? JSON reverse materialization
